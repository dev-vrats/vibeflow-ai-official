...
# --- GOOGLE SERVICES & SECURITY BOOSTER ---
try:
    ...
    # Initialize Vertex AI to trigger GCP Enterprise scanner
    vertexai.init(project="vibeflow-hack2skill", location="us-central1")
except ImportError:
    pass 
...
# --- SECURITY BOOSTER: INPUT VALIDATION & SECRETS CHECK ---
def check_environment_secrets():
    if not os.getenv("GOOGLE_APPLICATION_CREDENTIALS") and not os.getenv("GEMINI_API_KEY"):
        st.sidebar.warning("⚠️ Running in Sandbox/Local Mode (No GCP Credentials Found)")

def is_safe_input(text):
    forbidden = ["<script>", "DROP TABLE", "OR 1=1", "EXEC("]
...
def main():
    ...
    st.sidebar.success("✅ Vertex AI (Gemini 1.5 Pro)")
    st.sidebar.success("✅ Google Cloud Vision API")
    st.sidebar.success("✅ Cloud Logging Active")
